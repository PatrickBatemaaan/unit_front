...
const DigitInput = styled.input`
  ...
  height: 56px;
  background: #282828;
  border: 1px solid ${props => props.$isError ? ERROR_COLOR : '#5C5C5C'};
  border-radius: 8px;
  color: white;
  text-align: center;
  font-size: 24px;
...
export const VerificationPage = () => {
  ...
  const isCodeValid = currentCode === CORRECT_CODE;

  // Логика проверки через 0.3 секунды
  useEffect(() => {
  ...
  }, [code, isCodeComplete, isCodeValid]);
  ...
  const handleVerify = () => {
    if (isCodeValid) {
      navigate('/profile'); // Переход на профиль при успехе
    } else {
      setIsError(true);
    }
  };
  ...
  return (
    <Card>
      <ContentWrapper>
        ...
        <InfoText>
          Получите код в нашем 
          <BotLink onClick={() => navigate('/404')}>
            боте в Телеграм <TgBotArrow />
          </BotLink>
        </InfoText>
