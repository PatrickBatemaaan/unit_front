...
// Импорты твоих новых SVG из папки icons
...
const HeaderContainer = styled.header`
  width: 100%;
  height: 64px;
  background-color: #282828; /* Новый цвет из ТЗ */
  display: flex;
...
const LogoGroup = styled.div`
  position: absolute;
  left: 304px; /* Позиция из скрина 2 */
  top: 12px;
  display: flex;
  align-items: flex-start;
  gap: 17px; /* Высчитанный отступ между лого и первой буквой Ю */
  cursor: pointer;
`;

const StyledLogo = styled(UnitLogo)`
  width: 35px;
  height: 40px;
`;
...
const LettersWrapper = styled.div`
  display: flex;
  align-items: center;
  gap: 4.5px; /* Высчитанный отступ между самими буквами Ю Н И Т */
  margin-top: 11px; /* Выравнивание по Top: 23px (23 - 12 от родителя = 11) */
`;

// Применяем строгие размеры из скринов 3-6
...
const Nav = styled.nav`
  position: absolute;
  left: 50%; /* Центрируем, как ты просил */
  top: 22px;
...
const UserGroup = styled.div`
  position: absolute;
  right: 304px; /* Симметрично логотипу (1440 - 304) */
  top: 12px;
  display: flex;
  align-items: center;
  gap: 12px;
...
const Avatar = styled.div`
  ...
  height: 40px;
  border-radius: 50%;
  background-color: #D9D9D9; /* Цвет из профиля */
`;
...
export const Header = () => {
  ...
  // Проверяем, на странице авторизации ли мы
  ...
  return (
    <HeaderContainer>
      <HeaderInner>
        ...
        {/* Меню (скрыто на страницах авторизации) */}
        {!isAuthPage && (
          <Nav>
        ...
        {/* Правая часть: кнопка Войти ИЛИ Аватарка */}
        {isAuthPage ? (
